...
function showOverlay(): void {
  ...
  overlay.innerHTML = ok
    ? `<div class="card win"><h2>果蝇通关！</h2><p>Fly agent reached the flag in ${world.time.toFixed(1)}s · 果实 ${world.fruits}</p><button type="button" class="btn primary" id="again">再来一次 Retry</button></div>`
    : `<div class="card lose"><h2>跌入培养皿…</h2><p>Failed — 再试一次。Time ${world.time.toFixed(1)}s</p><button type="button" class="btn primary" id="again">重试 Retry</button></div>`;
